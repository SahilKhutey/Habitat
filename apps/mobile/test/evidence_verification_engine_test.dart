...
void main() {
  ...
  group('EvidenceVerificationEngine Phase 14 Tests', () {
    test('Rule 1: Valid Photo Proof passes all verification checks', () async {
      ...
      final result = await engine.verifyEvidence(capture,
          task: task, attemptId: 'attempt_01');
      expect(result.isPassed, isTrue);
      expect(result.failedRules.isEmpty, isTrue);
      expect(result.confidenceScore, greaterThan(0.9));
    });

    test('Rule 2: Video Proof with duration >= 3s passes verification',
        () async {
      final task = LocalTask(
        id: 'task_002',
        title: '15 Pushups',
      ...
      final result = await engine.verifyEvidence(capture,
          task: task, attemptId: 'attempt_02');
      expect(result.isPassed, isTrue);
    });
    ...
    test('Rule 3: Video Proof with duration < 3s is rejected', () async {
      ...
      final shortCapture = CaptureResult(
        ...
        byteSize: 1024 * 100,
        sha256Checksum:
            '3333444455556666777788889999000011112222aaaabbbbccccddddeeeeffff',
        durationSeconds: 2,
        capturedAt: DateTime.now(),
      );

      final result = await engine.verifyEvidence(shortCapture,
          task: task, attemptId: 'attempt_03');
    ...
    test('Rule 4: Anti-Replay duplicate SHA-256 hash is rejected', () async {
      ...
      const duplicateHash =
          '4444555566667777888899990000111122223333aaaabbbbccccddddeeeeffff';
      ...
      final res1 = await engine.verifyEvidence(capture1,
          task: task1, attemptId: 'att_1');
      expect(res1.isPassed, isTrue);

      // Second submission with exact same hash is rejected
      final res2 = await engine.verifyEvidence(capture1,
          task: task2, attemptId: 'att_2');
    ...
    test('Rule 5: Stale Proof captured > 10 min prior is rejected', () async {
      ...
      final staleCapture = CaptureResult(
        ...
        byteSize: 1024 * 200,
        sha256Checksum:
            '5555666677778888999900001111222233334444aaaabbbbccccddddeeeeffff',
        capturedAt: DateTime.now().subtract(const Duration(minutes: 15)),
      );

      final result = await engine.verifyEvidence(staleCapture,
          task: task, attemptId: 'att_stale');
      expect(result.isPassed, isFalse);
      expect(result.failedRules, contains('PROOF_EXPIRED_STALE'));
    });

    test(
        'Rule 6: Task category mismatch (Photo provided for Video-required task) is rejected',
        () async {
      ...
      final photoCapture = CaptureResult(
        ...
        byteSize: 1024 * 200,
        sha256Checksum:
            '6666777788889999000011112222333344445555aaaabbbbccccddddeeeeffff',
        durationSeconds: 0,
        capturedAt: DateTime.now(),
      );

      final result = await engine.verifyEvidence(photoCapture,
          task: physicalTask, attemptId: 'att_mismatch');
      expect(result.isPassed, isFalse);
