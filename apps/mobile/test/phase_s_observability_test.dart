...
void main() {
  ...
  group('Phase S: Diagnostic Identity & PII Sanitization', () {
    test(
        'S1 & S2: Context holds valid release metadata and breadcrumbs redact PII',
        () {
      ...
      telemetry.recordBreadcrumb('auth',
          'User user.name@domain.com from IP 192.168.1.50 initialized mission');
  ...
  group('Phase S: Structured Diagnostic Event Logging (S4, S7, S8, S9)', () {
    test('S7: Alarm Lifecycle Events are logged deterministically', () {
      telemetry.recordDiagnosticEvent(
          DiagnosticEventType.alarmScheduleRequested,
          metadata: {'alarmId': 'alm_101'});
      telemetry.recordDiagnosticEvent(DiagnosticEventType.alarmScheduleAccepted,
          metadata: {'alarmId': 'alm_101'});
      telemetry.recordDiagnosticEvent(DiagnosticEventType.alarmTriggered,
          metadata: {'alarmId': 'alm_101'});
      telemetry.recordDiagnosticEvent(DiagnosticEventType.notificationShown,
          metadata: {'channel': 'habitat_alarm_channel'});
      telemetry.recordDiagnosticEvent(DiagnosticEventType.missionOpened,
          metadata: {'missionId': 'task_101'});

      expect(telemetry.eventLog.length, equals(5));
      expect(telemetry.eventLog[0],
          equals(DiagnosticEventType.alarmScheduleRequested));
      expect(
          telemetry.eventLog[3], equals(DiagnosticEventType.notificationShown));
      expect(telemetry.eventLog[4], equals(DiagnosticEventType.missionOpened));
    });

    test(
        'S9: Camera & Proof Failures are classified with exact diagnostic types',
        () {
      telemetry
          .recordDiagnosticEvent(DiagnosticEventType.cameraPermissionDenied);
      ...
      expect(telemetry.eventLog,
          contains(DiagnosticEventType.cameraPermissionDenied));
    ...
    test('S8: Persistence Lifecycle & Recovery Events are tracked cleanly', () {
      telemetry
          .recordDiagnosticEvent(DiagnosticEventType.persistenceLoadStarted);
      telemetry.recordDiagnosticEvent(DiagnosticEventType.migrationStarted,
          metadata: {'from': 'v1', 'to': 'v3'});
      telemetry.recordDiagnosticEvent(DiagnosticEventType.migrationSuccess);
      telemetry
          .recordDiagnosticEvent(DiagnosticEventType.persistenceLoadSuccess);

      expect(telemetry.eventLog.length, equals(4));
      expect(telemetry.eventLog.first,
          equals(DiagnosticEventType.persistenceLoadStarted));
      expect(telemetry.eventLog.last,
          equals(DiagnosticEventType.persistenceLoadSuccess));
    });
  });

  group('Phase S: Safe Error Recovery & Core Invariants (S5, S19)', () {
    test(
        'S5 & S19: Failed proof or missing verification never produces false mission completion or fake XP',
        () async {
