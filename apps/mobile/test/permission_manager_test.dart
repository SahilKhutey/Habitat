...
void main() {
  group('PermissionManager Unit Tests', () {
    test('checkStatus() returns valid status across platform types', () async {
      ...
      final notifStatus =
          await manager.checkStatus(HabitatPermissionType.notifications);
      expect(notifStatus, equals(HabitatPermissionStatus.granted));

      final alarmStatus =
          await manager.checkStatus(HabitatPermissionType.exactAlarms);
      expect(alarmStatus, equals(HabitatPermissionStatus.granted));

      final cameraStatus =
          await manager.checkStatus(HabitatPermissionType.camera);
    ...
    test('requestPermission() updates denied permission to granted', () async {
      ...
      status =
          await manager.requestPermission(HabitatPermissionType.microphone);
