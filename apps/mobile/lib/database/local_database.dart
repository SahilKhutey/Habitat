...
class LocalTask {
  ...
  factory LocalTask.fromMap(Map<String, dynamic> map) => LocalTask(
        ...
        requiresPhoto:
            map['requiresPhoto'] == 1 || map['requiresPhoto'] == true,
        requiresVideo:
            map['requiresVideo'] == 1 || map['requiresVideo'] == true,
        requiresVerification: map['requiresVerification'] == 1 ||
            map['requiresVerification'] == true,
...
class LocalAlarm {
  ...
  factory LocalAlarm.fromMap(Map<String, dynamic> map) => LocalAlarm(
        ...
        retryIntervalMinutes:
            (map['retryIntervalMinutes'] as num?)?.toInt() ?? 5,
        maxRetries: (map['maxRetries'] as num?)?.toInt() ?? 6,
        nextTrigger: map['nextTrigger'] != null
            ? DateTime.parse(map['nextTrigger'])
            : null,
        createdAt: DateTime.parse(map['createdAt']),
      );
}

typedef LocalAttempt = LocalTaskAttempt;
...
class LocalTaskAttempt {
  ...
  final String
      status; // SCHEDULED, RINGING, AWAITING_ACTION, COMPLETED, FAILED, RETRY
  final DateTime triggeredAt;
  final DateTime? completedAt;

  DateTime get startedAt => triggeredAt;
  ...
  LocalTaskAttempt({
    ...
    DateTime? startedAt,
    this.completedAt,
  }) : triggeredAt = triggeredAt ?? startedAt ?? DateTime.now();
  ...
  factory LocalTaskAttempt.fromMap(Map<String, dynamic> map) =>
      LocalTaskAttempt(
        id: map['id'],
        taskId: map['taskId'],
        alarmId: map['alarmId'],
...
class LocalJournalEntry {
  ...
  factory LocalJournalEntry.fromMap(Map<String, dynamic> map) =>
      LocalJournalEntry(
        id: map['id'],
...
class LocalWaterEntry {
  ...
  LocalWaterEntry(
      {required this.id, required this.milliliters, required this.recordedAt});
...
class LocalNapEntry {
  ...
  int get durationMinutes =>
      (endedAt ?? DateTime.now()).difference(startedAt).inMinutes;
...
class DurableSyncEvent {
  ...
  factory DurableSyncEvent.fromMap(Map<String, dynamic> map) =>
      DurableSyncEvent(
        id: map['id'],
...
class LocalDatabase {
  ...
  void completeTask(String taskId) {
    ...
    if (task.requiresPhoto || task.requiresVideo) {
      final hasVerifiedProof = _proofs
              .any((p) => p.taskId == taskId && p.isVerified) ||
  ...
  List<LocalAlarm> getAlarmsForTask(String taskId) =>
      _alarms.values.where((a) => a.taskId == taskId).toList();
  ...
  void addWater({required int milliliters, DateTime? recordedAt}) {
    if (milliliters <= 0)
      throw ArgumentError.value(milliliters, 'milliliters', 'must be positive');
  ...
  void addMeal({required String type, String? notes, DateTime? recordedAt}) {
    ...
    if (!allowedTypes.contains(type))
      throw ArgumentError.value(type, 'type', 'must be a supported meal type');
  ...
  LocalNapEntry startNap({DateTime? startedAt}) {
    ...
    final entry = LocalNapEntry(
        id: const Uuid().v4(), startedAt: startedAt ?? DateTime.now());
  ...
  List<LocalWaterEntry> getWaterEntriesForDay(DateTime day) =>
      _waterEntries.where((entry) => _sameDay(entry.recordedAt, day)).toList();
  List<LocalWaterEntry> getWaterEntriesToday() =>
      getWaterEntriesForDay(DateTime.now());
  List<LocalMealEntry> getMealEntriesForDay(DateTime day) =>
      _mealEntries.where((entry) => _sameDay(entry.recordedAt, day)).toList();
  List<LocalNapEntry> getNapEntriesForDay(DateTime day) =>
      _napEntries.where((entry) => _sameDay(entry.startedAt, day)).toList();

  List<LocalWaterEntry> getAllWaterEntries() =>
      List.unmodifiable(_waterEntries);
  ...
  List<LocalJournalEntry> getAllJournalEntries() =>
      _journalEntries.values.toList()..sort((a, b) => b.date.compareTo(a.date));
  ...
  double getTodayWaterLiters() {
    ...
    if (todayLogs.isNotEmpty) {
      final totalMl = todayLogs.fold<double>(
          0.0, (sum, l) => sum + (l.unit == 'L' ? l.amount * 1000 : l.amount));
  ...
  int getTodayNapMinutes() {
    ...
    if (todayNapLogs.isNotEmpty) {
      return todayNapLogs.fold<int>(
          0, (sum, l) => sum + (l.durationMinutes ?? l.amount.toInt()));
    }
    final now = DateTime.now();
  ...
  Map<String, int> getDailyCompletions() {
    ...
    for (int i = 6; i >= 0; i--) {
      ...
      final completedCount = _attempts
          .where((a) => a.status == 'COMPLETED' && _sameDay(a.triggeredAt, day))
          .length;
  ...
  Map<String, double> getDailyWater() {
    ...
    for (int i = 6; i >= 0; i--) {
      ...
      final healthWater = _healthLogs
          .where((l) => l.type == 'WATER' && _sameDay(l.recordedAt, day));
      double totalLiters = 0.0;
      if (healthWater.isNotEmpty) {
        totalLiters = healthWater.fold<double>(0.0,
            (sum, l) => sum + (l.unit == 'L' ? l.amount : l.amount / 1000.0));
      } else {
        totalLiters =
            waterEntries.fold<int>(0, (sum, e) => sum + e.milliliters) / 1000.0;
      }
  ...
  Map<String, dynamic> getAppearanceSettings() =>
      Map.unmodifiable(_appearanceSettings);
  ...
  Map<String, dynamic> getSecuritySettings() =>
      Map.unmodifiable(_securitySettings);
  ...
  Map<String, int> getStorageBreakdown() {
    return {
      ...
      'health': _waterEntries.length * 40 +
          _mealEntries.length * 60 +
          _napEntries.length * 50,
  ...
  Map<String, dynamic> _migrateSchema(Map<String, dynamic> raw) {
    ...
    int version = (data['schemaVersion'] as num?)?.toInt() ??
    ...
    try {
      ...
      if (migrated['currentUser'] != null) {
        _currentUser = LocalUser.fromMap(
            Map<String, dynamic>.from(migrated['currentUser']));
      } else if (migrated['user'] != null) {
        _currentUser =
            LocalUser.fromMap(Map<String, dynamic>.from(migrated['user']));
      }
      ...
      if (migrated['attempts'] is List) {
        for (final item in migrated['attempts']) {
          _attempts
              .add(LocalTaskAttempt.fromMap(Map<String, dynamic>.from(item)));
        }
      }
      ...
      if (migrated['waterEntries'] is List) {
        for (final item in migrated['waterEntries']) {
          _waterEntries
              .add(LocalWaterEntry.fromMap(Map<String, dynamic>.from(item)));
        }
      }
      ...
      if (migrated['mealEntries'] is List) {
        for (final item in migrated['mealEntries']) {
          _mealEntries
              .add(LocalMealEntry.fromMap(Map<String, dynamic>.from(item)));
        }
      }
      ...
      if (migrated['napEntries'] is List) {
        for (final item in migrated['napEntries']) {
          _napEntries
              .add(LocalNapEntry.fromMap(Map<String, dynamic>.from(item)));
        }
      }
      ...
      if (migrated['healthLogs'] is List) {
        for (final item in migrated['healthLogs']) {
          _healthLogs
              .add(LocalHealthLog.fromMap(Map<String, dynamic>.from(item)));
        }
      }
      ...
      if (migrated['eventLogs'] is List) {
        for (final item in migrated['eventLogs']) {
          _eventLogs
              .add(LocalEventLog.fromMap(Map<String, dynamic>.from(item)));
        }
      }
      ...
      if (migrated['feedbackList'] is List) {
        for (final item in migrated['feedbackList']) {
          _feedbackList
              .add(LocalFeedback.fromMap(Map<String, dynamic>.from(item)));
        }
      }
      ...
      if (migrated['syncQueue'] is List) {
        for (final item in migrated['syncQueue']) {
          _syncQueue
              .add(DurableSyncEvent.fromMap(Map<String, dynamic>.from(item)));
        }
      }
      ...
      if (migrated['streak'] != null) {
        _streak =
            LocalStreak.fromMap(Map<String, dynamic>.from(migrated['streak']));
      }
      ...
      if (migrated['notificationSettings'] is Map) {
        _notificationSettings =
            Map<String, dynamic>.from(migrated['notificationSettings']);
      }
      if (migrated['appearanceSettings'] is Map) {
        _appearanceSettings =
            Map<String, dynamic>.from(migrated['appearanceSettings']);
      }
      if (migrated['privacySettings'] is Map) {
        _privacySettings =
            Map<String, dynamic>.from(migrated['privacySettings']);
      }
      if (migrated['securitySettings'] is Map) {
        _securitySettings =
            Map<String, dynamic>.from(migrated['securitySettings']);
      }
  ...
  void enqueueSyncEvent({
    ...
    final existingIdx =
        _syncQueue.indexWhere((e) => e.idempotencyKey == idempotencyKey);
    if (existingIdx < 0) {
  ...
  List<DurableSyncEvent> getPendingSyncEvents() =>
  ...
  // Reset MVP
  void resetAllData({bool populateDefaultTemplates = true}) {
    _currentUser = null;
    _tasks.clear();
    _alarms.clear();
    _attempts.clear();
