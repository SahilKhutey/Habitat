...
```
       ...
       Flutter/Swift        Flutter/Kotlin       Flutter Web
             ...
             │                    │                    │
             ▼                    ▼                    ▼
         SQLite / Postgres     Redis 7              MinIO S3
      (Relational DB)       (Queue & Cache)      (Object Storage)
```

## Source-of-Truth Hierarchy
1. **Product Rules & Recurrence**: Pure Dart Domain Engine (`packages/domain/`).
2. **User Data & XP Ledger**: SQLite / PostgreSQL (`users`, `streaks`, `xp_transactions`).
3. **Mission State**: Express Backend Core (`missions`, `mission_attempts`, `proofs`).
